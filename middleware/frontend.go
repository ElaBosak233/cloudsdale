package middleware

import (
	"github.com/gin-gonic/gin"
	"net/http"
	"os"
	"path"
	"strings"
)

func FrontendMiddleware(urlPrefix, root string) gin.HandlerFunc {
	fileServer := http.FileServer(http.Dir(root))
	if !strings.HasSuffix(urlPrefix, "/") {
		urlPrefix = urlPrefix + "/" // 如果不是以 / 结尾的，需要添加 /
	}
	staticServerPrefix := strings.TrimRight(urlPrefix, "/") // 生成静态文件服务的前缀
	return func(c *gin.Context) {
		if strings.HasPrefix(c.Request.URL.Path, "/api") || strings.HasPrefix(c.Request.URL.Path, "/docs") {
			c.Next()
		} else {
			filePath := path.Join(root, c.Request.URL.Path)
			_, err := os.Stat(filePath)
			if err != nil {
				http.ServeFile(c.Writer, c.Request, path.Join(root, "404.html"))
				c.Abort()
			} else {
				http.StripPrefix(staticServerPrefix, fileServer).ServeHTTP(c.Writer, c.Request)
				c.Abort()
			}
		}
	}
}
