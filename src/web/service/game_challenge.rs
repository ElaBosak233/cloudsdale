...
pub async fn create(
    ...
    match crate::model::game_challenge::create(req.into()).await {
        Ok(_) => {
            return Ok(());
        }
        Err(e) => {
...
pub async fn update(
    ...
    match crate::model::game_challenge::update(req.into()).await {
        Ok(_) => {
            return Ok(());
        }
        Err(e) => {
...
pub async fn delete(id: i64, challenge_id: i64) -> Result<(), Box<dyn Error>> {
    match crate::model::game_challenge::delete(id, challenge_id).await {
        Ok(_) => {
            return Ok(());
        }
        Err(e) => {
