...
pub async fn find(
    ...
    let (categories, total) = crate::model::category::find(req.id, req.name)
        .await
        .unwrap();
...
pub async fn create(
    ...
    match crate::model::category::create(req.into()).await {
...
pub async fn update(
    ...
    match crate::model::category::update(req.into()).await {
        Ok(_) => return Ok(()),
        Err(err) => return Err(Box::new(err)),
    }
}
...
pub async fn delete(id: i64) -> Result<(), Box<dyn Error>> {
    match crate::model::category::delete(id).await {
        Ok(_) => return Ok(()),
        Err(err) => return Err(Box::new(err)),
    }
}
