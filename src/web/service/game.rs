...
pub async fn find(
    ...
    let (games, total) =
        crate::model::game::find(req.id, req.title, req.is_enabled, req.page, req.size)
            .await
            .unwrap();
...
pub async fn create(
    ...
    match crate::model::game::create(req.into()).await {
...
pub async fn update(req: crate::model::game::request::UpdateRequest) -> Result<(), Box<dyn Error>> {
    match crate::model::game::update(req.into()).await {
        Ok(_game) => return Ok(()),
        Err(err) => return Err(Box::new(err)),
    }
}
...
pub async fn delete(id: i64) -> Result<(), Box<dyn Error>> {
    match crate::model::game::delete(id).await {
        Ok(_) => return Ok(()),
        Err(err) => return Err(Box::new(err)),
    }
}
